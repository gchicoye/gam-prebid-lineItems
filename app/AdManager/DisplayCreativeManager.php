...
use Google\AdsApi\AdManager\v202002\ThirdPartyCreative;
use Google\AdsApi\AdManager\v202002\Size;
use Google\AdsApi\AdManager\v202002\ApiException;

class DisplayCreativeManager extends Manager
{
	protected $ssp;
	...
	public function setUpCreatives($isSafeActive = false)
	{
		$safeframe = $isSafeActive == false ? false : true;
		...
		foreach ($creativeNameList as $creativeName) {
			if (empty(($foo = $this->getCreative($creativeName)))) {
				$foo = $this->createCreative($creativeName, $this->createSnippet($safeframe), $this->advertiserId, $safeframe);
			} else {
				$foo = $this->updateCreative($creativeName, $this->createSnippet($safeframe), $this->advertiserId, $safeframe);
			}
	...
	*/
	...
	private function createSnippet($safeframe)
	{
		if($this->type == "display"){
			...
			return $this->createVideoSnippet();
		}
